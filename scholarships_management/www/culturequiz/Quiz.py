...
@frappe.whitelist(allow_guest=True)
def get_questions_options(competition_title, system_lang='en'):
    quest_lst = get_translated_question(competition_title, system_lang)
    optn_lst = get_translated_option(competition_title, system_lang)

    question_option_dict = {}
    for indx in range(len(quest_lst)):
        question_option_dict[quest_lst[indx]] = optn_lst[indx]
...
def get_quiz_score(selected_options, competition_title, passing_score):
    ...
    selected_option_lst = get_source_text_for_option(selected_option_lst) # get source text of translted option..
